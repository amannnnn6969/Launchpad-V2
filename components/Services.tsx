...
function ServiceCard({ s, delay }: { s: typeof services[0]; delay: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: "1.8rem", marginBottom: "1.1rem", display: "block" }}>{s.icon}</span>
        ...
        <span style={{
          ...
          padding: "0.28rem 0.8rem", borderRadius: "2rem", border: "1px solid var(--border2)",
        }}>{s.tag}</span>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div ref={ref} className={`reveal ${inView ? "visible" : ""}`} style={{ textAlign: "center", marginBottom: "4rem" }}>
        <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "clamp(1.9rem, 3.5vw, 2.8rem)", fontWeight: 800, letterSpacing: "-0.03em", color: "var(--text)" }}>What we build for you</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.4rem" }}>
        {services.map((s, i) => <ServiceCard key={s.title} s={s} delay={i * 60} />)}
      </div>
    </div>
  );
}
