...
export default function Contact() {
  ...
  return (
    <div id="contact" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "2rem", padding: "5rem 3rem", maxWidth: 1200, margin: "0 auto 4rem", transition: "background 0.5s" }}>
      <div ref={ref} className={`reveal ${inView ? "visible" : ""}`} style={{ textAlign: "center", marginBottom: "4rem" }}>
        <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "clamp(1.9rem,3.5vw,2.8rem)", fontWeight: 800, letterSpacing: "-0.03em", color: "var(--text)" }}>Let&apos;s talk</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: "4rem", alignItems: "start" }}>
        ...
        <div className={`reveal ${inView ? "visible" : ""}`}>
          <h3 style={{ fontFamily: "Syne, sans-serif", fontSize: "1.4rem", fontWeight: 800, color: "var(--text)", marginBottom: "1rem" }}>Get your free audit</h3>
          <p style={{ color: "var(--text2)", lineHeight: 1.8, fontSize: "0.95rem", marginBottom: "2rem" }}>No pitch, no pressure. We&apos;ll look at your current online presence and give you an honest breakdown of what&apos;s working and what isn&apos;t.</p>
          {[["📧", "hello@launchpad.agency"], ["📞", "+91 98765 43210"], ["📍", "Mumbai, India — serving clients nationwide"]].map(([icon, text]) => (
            <div key={text} style={{ display: "flex", alignItems: "center", gap: "0.8rem", marginBottom: "1rem", color: "var(--text2)", fontSize: "0.9rem" }}>
              <div style={{ width: 36, height: 36, background: "var(--accent-glow)", border: "1px solid var(--border2)", borderRadius: "0.6rem", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem", flexShrink: 0 }}>{icon}</div>
              {text}
            </div>
          ))}
        </div>
          ...
          {submitted ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <button
                ...
                style={{ background: "var(--accent)", color: "#fff", border: "none", padding: "0.9rem 2.4rem", borderRadius: "2rem", fontSize: "0.95rem", fontWeight: 600, cursor: "pointer", fontFamily: "Inter, sans-serif", alignSelf: "flex-start", marginTop: "0.5rem", transition: "transform 0.2s, box-shadow 0.2s" }}
